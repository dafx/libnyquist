...
void mdctBackwardWithCuda(const var_t *in, var_t *out,
    ...
    size_t in_size = N * stride * sizeof(var_t);
    size_t out_size = N * sizeof(var_t);
    size_t trig_size = N * sizeof(var_t);
    size_t window_size = overlap * sizeof(var_t);
    size_t f2_size = N2 * sizeof(var_t);

    var_t *d_in = nullptr;
    var_t *d_out = nullptr;
    var_t *d_trig = nullptr;
    var_t *d_window = nullptr;
    var_t *d_f2 = nullptr;
    ...
    if ((err = cudaMemcpy(d_in, in, in_size, cudaMemcpyHostToDevice)) != cudaSuccess ||
        (err = cudaMemcpy(d_trig, trig, trig_size, cudaMemcpyHostToDevice)) != cudaSuccess ||
        (err = cudaMemcpy(d_window, window, window_size, cudaMemcpyHostToDevice)) != cudaSuccess) {
        std::cerr << "CUDA memcpy H2D failed: " << cudaGetErrorString(err) << std::endl;
        goto cleanup;
    }
    
    #ifdef DEBUG
    std::cout << "Launching kernel with blocks=" << numBlocks << ", threads=" << blockSize << std::endl;
    #endif

    mdctBackwardKernel<<<numBlocks, blockSize>>>(
        d_in, d_out, d_trig, d_window, d_f2,
        N, N2, N4, overlap, shift, stride, sine
    );
    ...
    if ((err = cudaMemcpy(out, d_out, out_size, cudaMemcpyDeviceToHost)) != cudaSuccess) {
